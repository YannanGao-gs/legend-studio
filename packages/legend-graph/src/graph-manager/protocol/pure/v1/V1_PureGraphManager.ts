...
export class V1_PureGraphManager extends AbstractPureGraphManager {
  ...
  async buildGraphForQuery(
    ...
    try {
      ...
      buildState.setMessage(`Building services and execution environments...`);
      await this.buildServices(graph, buildInputs, options);
      stopWatch.record(
        GRAPH_MANAGER_EVENT.GRAPH_BUILDER_BUILD_SERVICES__SUCCESS,
      );
