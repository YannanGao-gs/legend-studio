...
export const V1_getFullReturnTypePath = (val: V1_GenericType): string => {
  if (val.rawType instanceof V1_PackageableType) {
    return val.rawType.fullPath;
  }
  throw new UnsupportedOperationError('Full Generics novvvvt supported Yet');
};
...
const V1_buildFunctionSignatureSuffix = (
  ...
  `_${func.parameters
    ...
    .join('_')}_${V1_getFullReturnTypePath(func.returnGenericType)
    .split(ELEMENT_PATH_DELIMITER)
...
const V1_buildFunctionPrettyName = (
  ...
  `${
    ...
    .join(
      ', ',
    )}): ${V1_getFullReturnTypePath(element.returnGenericType)}[${generateMultiplicityString(
...
export const V1_buildFunctionInfoAnalysis = (
  ...
  const functionInfos = functionProtocols.map((func) => {
    ...
    functionInfo.returnType = V1_getFullReturnTypePath(func.returnGenericType);
