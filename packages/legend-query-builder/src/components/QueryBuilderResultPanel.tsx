...
const QueryBuilderGridResultContextMenu = observer(
  forwardRef<
    HTMLDivElement,
    {
      ...
      dataGridApi: DataGridApi<QueryBuilderTDSResultCellDataType>;
    }
    ...
    const findRowFromRowIndex = (rowIndex: number): string => {
      ...
      Object.entries(
        dataGridApi.getRenderedNodes().find((n) => n.rowIndex === rowIndex)
          ?.data as unknown as [string, QueryBuilderTDSResultCellDataType][],
      ).forEach((entry) => {
        if (entry[0] !== 'rowNumber') {
          valueArr.push(
            entry[1] as unknown as string | number | boolean | null | undefined,
          );
        }
      });
...
): PlainObject<QueryBuilderTDSRowData>[] => {
          ...
          for (let x: number = startRow; x <= endRow; x++) {
            ...
            if (curRowData) {
              for (const col of selectedColumns) {
                const valueAndColumnId = {
                  value: (Object.entries(curRowData) as [string, unknown][])
