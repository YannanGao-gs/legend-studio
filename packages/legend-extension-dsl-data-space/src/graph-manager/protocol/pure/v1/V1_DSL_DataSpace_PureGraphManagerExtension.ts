...
import { retrieveDataSpaceAnalysisFromFileGeneration } from '../../../action/analytics/DataSpaceAnalysisHelper.js';
...
export class V1_DSL_DataSpace_PureGraphManagerExtension extends DSL_DataSpace_PureGraphManagerExtension {
  ...
  async analyzeDataSpaceCoverage(
    ...
    const result = retrieveDataSpaceAnalysisFromFileGeneration(
    ...
    const dataspaceCoveragePartition = dataspaceArtifacts?.map((artifact) =>
      V1_deserializeDataspaceCoveragePartition(artifact),
    );
  ...
  async buildDataSpaceAnalytics(
    ...
    if (resolvedMappingPath) {
      const mappingModelCoverageAnalysisResult =
        mappingAnalysisCoveragePartitionMap.get(
          resolvedMappingPath,
        )?.analysisResult;
